...
trailing_space = true       # added 2026-09-10, FROM USE. End every transcript
                            # with exactly one space. Dictation lands with the
                            # caret flush against the last character, so the
                            # next thing typed runs into the last word — and
                            # every dictation is followed by something, of which
                            # only one wants no separator. ON by default: the
                            # cost is one character the user can backspace,
                            # against a defect they must notice and repair every
                            # time. Runs LAST — `collapse_whitespace` strips
                            # trailing whitespace and runs first. Leaves text
                            # already ending in whitespace alone, so a
                            # `spoken_commands` paragraph break does not gain a
                            # space on the line just left, and leaves an empty
                            # transcript empty rather than manufacturing a
                            # keystroke out of a guard refusal.
...
- **A recording affordance with more presence than a menu-bar glyph** (added
  ...
  *tacky, too square, too big, and it should have no text.* Now **72×22 with a
  fully round 11 px radius** and seven bars driven by the live RMS of the audio
  being captured.

  > **Correction, 2026-09-10.** This paragraph read *112×26 with a fully round
  > 13 px radius* from 2026-09-03 until today. **That was never the shipped
  > geometry.** `ui/overlay.py` has said `_WIDTH 72`, `_HEIGHT 22`,
  > `CORNER_RADIUS = _HEIGHT / 2.0` since the panel was built — the 112×26
  > figures describe an intermediate size that was tightened before the commit
  > landed, and the prose was not updated with the constants.
  >
  > Found by the choice-cartographer sentinel on 2026-09-10, which noticed that
  > `docs/superpowers/specs/overlay-controls.md` re-derives from the code and
  > arrives at a different number seventy lines below this one — so §5.4 was
  > publishing two incompatible geometries, both added by the same author
  > within a week, neither annotated. Corrected here rather than quietly,
  > because a silent in-place edit destroys the evidence that the document once
...
#### Overlay controls and the transcribing state (specified 2026-09-10)

**Specified, not built.** Full spec:
[`docs/superpowers/specs/overlay-controls.md`](docs/superpowers/specs/overlay-controls.md).
Sequenced after the Phase 4 gate — lane 2's 6/6 describes the panel as it exists
now, and rebuilding it before lane 6 would leave the install gate measuring a
panel nobody has used for more than a day.

A latched session gains an **✕** and a **✓** beside the waveform: discard, and
finish-and-process. They appear only when latched, because push-to-talk's
release already *is* the ✓ and its hand on the key already answers the question.

**The panel persists through TRANSCRIBING, and the waveform does not.** The
paragraph above forbids a panel that means *recording* outliving the microphone;
it does not forbid the panel. At the instant capture closes the bars are
**replaced in one frame** — never cross-faded, since a dimming waveform is still
a waveform — by a single dot whose motion is time-driven and carries no
audio-derived component. The invariant, and it is testable: **no audio-reactive
element is visible while the microphone is closed.**

**Two costs, accepted rather than removed.** The panel stops ignoring mouse
events, so clicks inside a 114 × 22 region do not reach the application beneath
it; that goes in the README's known-costs section, under the rule that put the
clipboard leak there. And **there is no Escape equivalent**: §7.3's hotkey tap
watches `flagsChanged` only and refuses `keyDown` because a tap that watched it
would see every character typed. The ✕ is the only cancel affordance the product
has, which raises its importance rather than lowering it.

The double-tap no longer blinks the panel. `abort_session` reports `IDLE`
between the two presses, which is an event that did not happen — the microphone
never closed. `DictationController` gains `restart_session`, which discards the
first tap's fragment without leaving `RECORDING`.
...
**Amended again 2026-09-14, because the first fix did not work.** The request
half chosen was `CGRequestPostEventAccess`, the obvious twin of the preflight
the check uses. On **macOS 26.6** it registered nothing: verifiably called, no
dialog, the pane still empty. The call that does the work is
`AXIsProcessTrustedWithOptions({kAXTrustedCheckOptionPrompt: True})` — the
older route to the same grant, and the one that presents the dialog carrying
"Open System Settings". It lives in `HIServices`, which is why
`pyobjc-framework-ApplicationServices` moves from the `gate` extra to a runtime
dependency; a plain `pip install .` previously had no route to the one API that
can put the user in the pane the remediation names. The `CGRequest*` call is
kept only as a fallback for an install assembled without it, and is **not**
...
│   ├── tier.py                   # the install-time timed check (§7.2)
│   ├── guard.py                  # the collapse guard (§5.7)
│   ├── launcher.py               # the Desktop launcher `manu install` writes
│   ├── assets/                   # shipped in the wheel: the reference clip,
│   │                             # and start-amanuensis.command
│   ├── models/
...
| Objections — `overlay-controls` | `docs/superpowers/objections/overlay-controls.md` | 12 objections — **all accepted** |
...
| Choice stories — `overlay-controls` | `docs/superpowers/stories/overlay-controls.md` | 11 stories — **all accepted** |
...
| 2026-09-08 | **`manu install` writes a double-clickable launcher, and the launcher is one file that works from three places** (§9 Phase 4, README). The product is a terminal command and §4's user is not universally someone who will open a terminal to start a dictation daemon each morning; a `.command` file is the smallest thing that removes that step without pretending to be an application. **It is not an `.app` and does not change that §5.4 still holds the bundle in reserve** — macOS attaches Accessibility and Input Monitoring to whatever launches the process, so double-clicking makes *Terminal.app* the grant holder rather than Amanuensis, which the README now says in both places it matters. The launcher moved into `src/amanuensis/assets/` so it ships in the wheel (`scripts/` is a symlink to it, so a checkout keeps the familiar path and there is one file rather than two that drift). It resolves its own location through any symlink chain and then walks **up** for a checkout rather than assuming a depth, which is what lets the same bytes serve `scripts/`, `assets/`, and a Desktop with no checkout above it. **One recorded path remains and it is written by a program, never typed:** an installed copy has no tree to point at and a Finder launch inherits no shell profile, so `manu` is neither beside it nor on `PATH` — `manu install` records where it put itself, rewrites that on re-run, and the launcher names `manu install` as the repair when the path goes stale instead of printing `cannot find` as its predecessor did. Refuses to overwrite a file it did not write (marker line) or to replace a developer's `--link` symlink with a copy, because that would silently downgrade *follows the tree* to *snapshot of the tree*. Two defects found by running it rather than reading it: the marker check read line one, which is the shebang, so `manu install` refused to replace **its own** launcher; and a Desktop copy adopted any checkout that happened to sit above it, which for a user with a clone at `$HOME` would have put a stranger's `src` on `PYTHONPATH` ahead of the environment they installed. |
