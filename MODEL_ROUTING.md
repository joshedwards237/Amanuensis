...
| assessor | Flagship | Mapping scattered repository evidence onto a literacy level is interpretive, not mechanical — the same artefacts support several readings, and the judgment is which one the evidence actually bears |
| governance-auditor | Flagship | Semantic drift detection, constraint falsifiability scoring, and three-frame alignment all require reading intent behind wording; the failure mode of a weaker model here is a confident audit that misses the drift it exists to catch |
| harness-auditor | Balanced | Comparing declared enforcement in HARNESS.md against what exists is largely structured verification, but judging whether a declaration is *honestly* satisfied needs more than string matching |
| harness-enforcer | Balanced | Executes constraints through a unified interface — deterministic tools run themselves; the agent-based reviews are checklist work a mid-tier model handles reliably |
| harness-gc | Balanced | Periodic entropy sweeps over documented rules — staleness, dead code, convention drift, dependency currency; structured, but each rule needs a judgment call on what counts as stale |
| harness-discoverer | Efficient | Detecting stack, linters, CI config, and test frameworks is pattern-matching against known file signatures — mechanical, high-throughput, no reasoning depth required |
